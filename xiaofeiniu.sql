SET NAMES UTF8;          /* 设置名称为UTF8 */
DROP DATABASE IF EXISTS  /* 如果存在就丢弃xiaofeiniu */
xiaofeiniu;
CREATE DATABASE xiaofeiniu  /* 创建数据库xiaofeiniu */
CHARSET=UTF8;   /* 表示当前文档的字符集是采用utf-8的字符,也就是我们常说英文字符集 */
USE xiaofeiniu;

/*管理员*/
CREATE TABLE xfn_admin(   /* 创建表 xfn_admin */
  aid INT PRIMARY KEY AUTO_INCREMENT,  /*管理员编号 aid int类型数据 设置主键 自动增长*/
  aname VARCHAR(32) UNIQUE, /*管理员用户名 aname 最大长度32位数字 唯一约束库表中只能有一个aname */
  apwd VARCHAR(64) /* 管理员密码 加密存储 */
);
INSERT INTO xfn_admin VALUES /* 插入数据到 xfn_admin表中 */
(NULL, 'admin', PASSWORD('123456')), /* PASSWORD 加密 */
(NULL, 'boss', PASSWORD('999999'));

/*项目全局设置*/
CREATE TABLE xfn_settings( /* 创建表 xfn_settings */
...
/*桌台预定信息*/
CREATE TABLE xfn_reservation( /* 创建表 xfn_reservation */
...
CREATE TABLE xfn_dish(
  ...
  FOREIGN KEY(categoryId) REFERENCES xfn_category(cid)
	/* 外键categoryId 参考表 xfn_category表中的(cid) */
);
...
CREATE TABLE xfn_order_detail(
  ...
  FOREIGN KEY(dishId) REFERENCES xfn_dish(did), 
	/* 外键 dishId 参考 xfn_dish的表(did)*/
  FOREIGN KEY(orderId) REFERENCES xfn_order(oid)
	/* 外键 orderId 参考 xfn_order的表(oid)*/
);
